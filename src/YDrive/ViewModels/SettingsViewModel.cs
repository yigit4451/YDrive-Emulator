...
public class SettingsViewModel : INotifyPropertyChanged
{
    ...
    private void UploadBios()
    {
        ...
        {
            Title = System.Windows.Application.Current?.TryFindResource("Dlg_BiosTitle") as string ?? "Please select BIOS files (.bin or .zip)",
            Filter = System.Windows.Application.Current?.TryFindResource("Dlg_BiosFilter") as string ?? "BIOS File|*.zip;*.bin|All Files|*.*"
        };
    ...
    private void DeleteAllBios()
    {
        ...
        string title = System.Windows.Application.Current?.TryFindResource("Msg_DeleteAllBiosTitle") as string ?? "Delete BIOS Files";
        string message = System.Windows.Application.Current?.TryFindResource("Msg_DeleteAllBiosConfirm") as string ?? "Are you sure you want to delete all SEGA CD BIOS files?";
        ...
        if (result == System.Windows.MessageBoxResult.Yes)
        {
            ...
            {
                string formatStr = System.Windows.Application.Current?.TryFindResource("Msg_BiosDeleteError") as string ?? "An error occurred while deleting BIOS files:\n{0}";
                System.Windows.MessageBox.Show(
                    string.Format(formatStr, ex.Message), 
                    System.Windows.Application.Current?.TryFindResource("Msg_Error") as string ?? "Error", 
