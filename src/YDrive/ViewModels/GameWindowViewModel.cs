...
public class GameWindowViewModel : INotifyPropertyChanged, IDisposable
{
    ...
    private void TryInitialize()
    {
        ...
        if (_game.SystemType == SystemType.SegaCD)
        {
            ...
            {
                string biosMsg = Application.Current?.TryFindResource("Msg_BiosMissing") as string ?? "Missing BIOS files detected. Please upload BIOS files from the Settings menu and try again.";
                string biosTitle = Application.Current?.TryFindResource("Msg_BiosMissingTitle") as string ?? "Game Cannot Start";
                MessageBox.Show(biosMsg, biosTitle, MessageBoxButton.OK, MessageBoxImage.Warning);
                StatusText = Application.Current?.TryFindResource("Msg_BiosMissingStatus") as string ?? "BIOS Missing";
                return;
            }
        }
            ...
            switch (result)
            {
                ...
                case LibretroCore.LoadStateResult.MismatchedRom:
                   msg = Application.Current?.TryFindResource("Msg_StateMismatch") as string 
                          ?? "❌ This save belongs to a different game!";
                    break;
                default:
