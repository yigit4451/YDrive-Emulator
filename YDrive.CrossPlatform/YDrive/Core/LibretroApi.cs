...
//  NativeLibrary.Load ile DLL/.SO yükler, fonksiyon pointer'larını bağlar
// ─────────────────────────────────────────────────────────────
...
public sealed class LibretroApi : IDisposable
{
    ...
    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    public delegate short RetroSetInputStateDelegate(RetroInputStateDelegate cb);
    ...
    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    [return: MarshalAs(UnmanagedType.I1)]
    public delegate bool RetroLoadGameSpecialDelegate(uint gameType, ref RetroGameInfo info, nuint numInfo);
    ...
    public void LoadCore(string dllPath)
    {
        ...
        if (OperatingSystem.IsAndroid())
        {
            ...
            try
            {
                nativePath = App.PlatformService?.GetNativeLibraryPath(dllPath)
                          ?? App.PlatformService?.GetNativeLibraryPath("genesis_plus_gx_libretro_android")
                          ?? App.PlatformService?.GetNativeLibraryPath("picodrive_libretro_android")
                          ?? App.PlatformService?.GetNativeLibraryPath("genesis_plus_gx_libretro");
            }
            catch { }
            ...
            if (!string.IsNullOrEmpty(nativePath) && File.Exists(nativePath))
            {
                try
                {
                    _libraryHandle = NativeLibrary.Load(nativePath);
                }
                catch { }
            }
            ...
            if (_libraryHandle == IntPtr.Zero)
            {
                string[] candidateLibs = {
                    ...
                    "libpicodrive_libretro.so",
                    "genesis_plus_gx_libretro_android",
                    "picodrive_libretro_android",
                    "genesis_plus_gx_libretro",
                    "picodrive_libretro"
                };

                foreach (var candidate in candidateLibs)
                {
                    try
                    {
            ...
            if (_libraryHandle == IntPtr.Zero)
            {
                throw new DllNotFoundException($"Genesis Plus GX / PicoDrive Libretro çekirdeği yüklenemedi: {nativePath ?? dllPath}");
            }
        }
        else
        {
